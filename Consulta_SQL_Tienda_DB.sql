SELECT 
    ...
    (
        ...
        FROM ps3vx7_category_product cp_url
        JOIN ps3vx7_category c_url
            ON c_url.id_category = cp_url.id_category
            AND c_url.id_parent NOT IN (0,1,2)
        JOIN ps3vx7_category_lang cl_url
            ON cl_url.id_category = c_url.id_category AND cl_url.id_lang = 1
        WHERE cp_url.id_product = p.id_product
          AND c_url.id_parent NOT IN (0,1,2)
        ORDER BY c_url.id_category DESC
        LIMIT 1
    ) AS url_producto,
