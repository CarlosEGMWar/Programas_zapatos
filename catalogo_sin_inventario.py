"""
Generador de catalogo PDF para zapatos EN PROCESO.
- Recorre ROOT_DIR buscando carpetas "En Proceso"
- Cada subcarpeta numerada dentro = un producto (SKU = numero)
- Titulo del bloque = carpeta padre de "En Proceso" (con acentos corregidos)
- Fotos desde la subcarpeta HD/
- Sin inventario, sin tallas: solo fotos + SKU + categoria + precio
- Varios modelos por pagina (minimo 3 si caben, foto fija = PHOTO_W cuadradas)
- Indice jerarquico por categoria, la hoja es link al primer producto
"""
...
def obtener_precio(categoria: str, sku: str) -> int:
    """Devuelve el precio CLP para una carpeta dado su SKU y categoría."""
...
def buscar_productos() -> list:
    """
    Recorre ROOT_DIR buscando carpetas 'En Proceso'.
    Por cada subcarpeta numerada dentro que tenga una carpeta HD con imagenes,
    crea un dict de producto.
    Devuelve lista ordenada por categoria y luego por numero de SKU.
    """
    productos = []
...
def calcular_ph(n_imgs: int) -> float:
    """Altura de foto ajustada para que el bloque nunca supere MAX_BLK_H."""
...
def calcular_bloque_h(n_imgs: int) -> float:
    """Altura total del bloque, garantizando que cabe en media pagina."""
...
def dibujar_botones_canales(c: canvas.Canvas, sku: str,
    ...
    """
    Dibuja la barra de canales entre el header y las fotos.
    zona_y  = coordenada Y inferior de la zona (igual que btn_bar_y)
    zona_h  = altura de la zona (= BTN_BAR_H)
    """
...
def agrupar_en_paginas(productos: list) -> list:
    """Maximo 2 modelos por pagina; nueva pagina al cambiar categoria."""
    if not productos:
        return []
    pages   = []
    current = []
...
def dibujar_indice(cv: canvas.Canvas, productos: list):
    """
    Indice jerarquico: solo categorias.
    La fila hoja = categoria, clickeable al primer producto de ese grupo.
    """
    W, H = A4
    M    = 12 * mm
