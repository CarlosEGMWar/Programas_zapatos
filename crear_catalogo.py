"""
Generador de catalogo PDF para calzado.
...
def agrupar_en_paginas(productos: list) -> list:
    ...
    - Modelos con 1-2 colores: se agregan greedily mientras quepan
      en altura disponible (sin reducir el tamano de las fotos).
    """
    if not productos:
        return []
    pages   = []
...
def dibujar_indice(cv: canvas.Canvas, productos_info: list):
    ...
    al primer producto de ese grupo. Sin filas individuales por producto.
    """
    W, H = A4
    M    = 12 * mm
...
def dibujar_portada(cv: canvas.Canvas):
    """Pagina 1: portada del catalogo con titulo y boton de enlace."""
    W, H = A4
