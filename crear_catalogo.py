"""
Generador de catalogo PDF para calzado — version vendedor/revendedor interno.

Publico objetivo: vendedores y revendedores que necesitan ver precios al por mayor
y acceder directamente a la tienda online para hacer pedidos.

Caracteristicas principales:
- Lee el inventario CSV: Inventario_web_precios_al_vendedor.csv
- Genera hasta 2 modelos por pagina (1 si el modelo ocupa mucho espacio)
- PNG con transparencia -> fondo blanco (no negro)
- Imagenes comprimidas a JPEG para reducir peso del PDF
- Chips de talla en DORADO = links clickeables a la tienda online
- Chips de talla en BLANCO/GRIS = sin stock en tienda online
- Boton "Ver en tienda" por modelo
- Botones de WhatsApp, Instagram y Facebook
- Indice jerarquico por categoria con links internos al PDF

Archivo de salida: catalogo.pdf (en la misma carpeta que este script)
"""
...
#INVENTARIO = ROOT_DIR / "Inventario/Inventario_web_now.csv"
INVENTARIO = ROOT_DIR / "Inventario/Inventario_web_precios_al_vendedor.csv"
OUTPUT_PDF = SCRIPT_DIR / "catalogo.pdf"
...
def buscar_hd(root: Path, numero: str):
    ...
    llamada 'HD', devuelve la ruta a esa subcarpeta HD. Esta estructura corresponde
    a: [Categoria]/[numero]/HD/
...
def preparar_imagen(img_path: Path, max_px: int = 700) -> tuple:
    ...
    3. Convertir a JPEG: guarda en buffer en memoria con calidad 60 y optimizacion
       activada, lo que reduce significativamente el peso final del PDF.

    Las imagenes procesadas se almacenan en el cache _img_cache usando la tupla
    (ruta, max_px) como clave, por lo que llamadas repetidas con los mismos
    parametros no vuelven a leer ni procesar el archivo.

    Parametros:
        img_path (Path): Ruta al archivo de imagen original (.png, .jpg o .jpeg).
        max_px (int): Dimension maxima en pixeles (ancho o alto) tras redimensionar.
                      Por defecto 700 px para la carga inicial; se usa 200-350 px
                      al dibujar en el PDF para reducir aun mas el peso.

    Devuelve:
        tuple: (ImageReader, ancho_px, alto_px) donde ImageReader es el objeto
               listo para pasar a canvas.drawImage(), y los enteros son las
               dimensiones en pixeles de la imagen procesada.
    """
...
def dibujar_imagen(c: canvas.Canvas, img_path: Path,
    ...
    Parametros:
        ...
        max_px (int): Resolucion maxima para preparar la imagen. Por defecto 200 px,
                      suficiente para la visualizacion en PDF a tamano A4.
    """
    try:
...
def rounded_rect(c: canvas.Canvas, x, y, w, h, r=2*mm,
    ...
    Permite especificar color de relleno, color de borde y grosor de linea de
    forma independiente. Se usa para dibujar chips de talla, botones de redes
    sociales, el boton "Ver en tienda" y otros elementos visuales del catalogo.
    ...
    Observacion:
        Si tanto fill como stroke son None, el rectangulo se dibuja pero
        permanece invisible (sin relleno ni borde).
    """
    if fill:
        c.setFillColor(fill)
    if stroke:
...
def dibujar_bloque(c: canvas.Canvas, filas: list, carpetas_hd: dict,
                   ...
                   photo_h: float = None):
    """
    Dibuja el bloque completo de un modelo en el lienzo PDF.
    ...
    - Columna izquierda:
        * SKU del modelo.
        * Boton "Ver en tienda" (link a la URL del producto, solo si existe).
        * Seccion "Tallas disponibles": chips por color, en DORADO si tienen
          link a la tienda online, en BLANCO/GRIS si no tienen stock en tienda.
        * Seccion "Ficha tecnica": filas de etiqueta-valor con los campos del CSV.
        * Botones de redes sociales: WhatsApp (con SKU en el mensaje), Instagram, Facebook.
    - Linea separadora en la parte inferior del bloque.

    El sistema de coordenadas de ReportLab tiene el origen (0,0) en la esquina
    inferior izquierda de la pagina. bloque_y es la coordenada Y del borde INFERIOR
    del bloque, y bloque_h es la altura total hacia arriba desde ese punto.

    Parametros:
        c (canvas.Canvas): Lienzo PDF sobre el que dibujar.
        filas (list): Lista de filas del CSV que corresponden al mismo ref_modelo.
                      Cada fila es un dict con todas las columnas del inventario.
        carpetas_hd (dict): Diccionario {numero_carpeta: Path_HD} con las carpetas
                            de imagenes encontradas para este modelo.
        ...
        is_first_on_page (bool): Si True, el header se extiende hasta el borde superior
                                 de la pagina para cubrir el margen. Por defecto False.
        photo_h (float): Altura de cada foto en puntos. Si None, usa PHOTO_W (cuadradas).
    """
    bw = PAGE_W - 2 * MARGIN     # ancho util del bloque

    base = filas[0]

    # Agrupar tallas y refs por color
    color_tallas: dict = defaultdict(list)
    talla_url: dict = {}   # (color, talla) -> url_producto
    for f in filas:
        col = val(f.get("color", "")) or "Sin color"
        t   = val(f.get("talla", ""))
        u   = val(f.get("url_producto", ""))
        if t:
    ...
    # El color con el ref_combinacion mas bajo va a la carpeta de numero mas bajo.
    color_min_ref: dict = {}
    for f in filas:
        col = val(f.get("color", "")) or "Sin color"
        try:
    ...
    # Mapping carpeta->color conservado para posible uso futuro (ej. tallas por color)
    carpeta_color = {num: (colores_orden[i] if i < len(colores_orden) else f"Var. {num}")
                     for i, num in enumerate(nums_carpeta)}
    _ = carpeta_color  # suprimir warning: se conserva para la columna izquierda

    # ── Mini-header ───────────────────────────────────────────────────────
    hdr_y = bloque_y + bloque_h - HDR_H
    # Rect siempre ancho completo; si es el primer bloque sube hasta el borde
    ...
    # Layout: cada color = banda horizontal, filas de N_LARGE fotos.
    # Todas del mismo tamaño. Gap fijo; si la ultima fila tiene menos fotos
    # se dibujan desde la izquierda sin estirar (igual tamano y separacion).

    N_LARGE   = 3         # fotos por fila
    FOTO_GAP  = 0.5 * mm # gap entre fotos (horizontal y vertical) — local, no usado ahora

    n_grupos = len(carpetas_hd)
    all_imgs = {num: imagenes_en_hd(hd) for num, hd in carpetas_hd.items()}

    # Numero de filas necesarias (max 2 ya que nunca habra mas de 6 fotos)
    max_rows = 2 if any(len(v) > N_LARGE for v in all_imgs.values()) else 1

    # photo_h viene del caller (calculado por pagina); fallback = cuadrado
    if photo_h is None:
        photo_h = PHOTO_W
    ...
    # ── COLUMNA IZQUIERDA ─────────────────────────────────────────────────
    lx  = bloque_x
    cy  = content_top - 3 * mm   # padding inicial bajo el header
    ...
    def sec_title(txt, yy):
        ...
        Devuelve:
            float: Nueva coordenada Y del cursor, ya desplazada hacia abajo para
                   que el siguiente elemento se dibuje en la posicion correcta.
        """
        c.setFillColor(C_ACCENT)
    ...
    def row(label, value, yy, label_w=25*mm):
        ...
        en fuente normal. Si el valor es demasiado largo para el espacio disponible,
        se trunca con puntos suspensivos para evitar desbordamiento.

        Parametros:
            label (str): Nombre del campo a mostrar (ej: "Empeine", "Suela").
            value (str): Valor del campo (ej: "Cuero genuino", "Goma").
            ...
            label_w (float): Ancho reservado para la etiqueta. Por defecto 25 mm.
                             El valor se dibuja a partir de lx + label_w.

        Devuelve:
            float: Nueva coordenada Y del cursor, desplazada hacia abajo 4.8 mm
    ...
    # Boton de enlace web (si existe url_producto)
    url = val(base.get("url_producto", ""))
    if url:
        btn_w = 38 * mm
        btn_h = 6  * mm
        btn_x = lx
        btn_y = cy - btn_h
        # Fondo del boton
        rounded_rect(c, btn_x, btn_y, btn_w, btn_h, r=2*mm, fill=C_ACCENT)
        # Icono de flecha + texto
        c.setFillColor(C_DARK)
        c.setFont("Helvetica-Bold", 7.5)
        c.drawCentredString(btn_x + btn_w / 2, btn_y + 1.8*mm, "Ver en tienda  >")
        # Area clickeable
        c.linkURL(url, (btn_x, btn_y, btn_x + btn_w, btn_y + btn_h),
                  thickness=0)
        cy -= btn_h + 4 * mm
    else:
        cy -= 2 * mm

    # Tallas por color (primero)
    cy = sec_title("Tallas disponibles", cy)

    # Aviso de botones clickeables (una sola vez por modelo)
    has_links = any(talla_url.values())
    if has_links:
        c.setFillColor(C_MID)
        c.setFont("Helvetica-Oblique", 5.5)
        c.drawString(lx, cy, "Toca una talla para comprar ese modelo")
        cy -= 3.5 * mm

    for color, tallas in color_tallas.items():   # orden de aparicion en CSV
        ...
        # Chips de talla (botones linkables si tienen URL)
        chip_w  = 17 * mm
        chip_h  = 5  * mm
        chip_g  = 2  * mm
        cx_chip = lx
        for talla in tallas:
            if cx_chip + chip_w > lx + LEFT_W + 1*mm:
                ...
                if cy < content_bot + 5*mm:
                    break
            url_t = talla_url.get((color, talla), "")
            if url_t:
                # Boton dorado con borde
                rounded_rect(c, cx_chip, cy - chip_h, chip_w, chip_h,
                             r=2*mm, fill=C_ACCENT, stroke=C_ACCENT)
                c.setFillColor(C_DARK)
            else:
                rounded_rect(c, cx_chip, cy - chip_h, chip_w, chip_h,
                             r=2*mm, fill=C_WHITE, stroke=C_LINE)
                c.setFillColor(C_DARK)
            c.setFont("Helvetica-Bold" if url_t else "Helvetica", 6.5)
            # Tomar solo el numero: antes del primer " - ", quitar parentesis
            num_t = talla.split(" - ")[0].replace("(", "").replace(")", "").strip()
            c.drawCentredString(cx_chip + chip_w/2, cy - chip_h + 1.5*mm, num_t)
            if url_t:
                c.linkURL(url_t,
                          (cx_chip, cy - chip_h, cx_chip + chip_w, cy),
                          thickness=0)
    ...
    # Ficha tecnica (despues de tallas)
    ...
    SOCIAL_MIN_H = 6*mm + 3.5*mm + 6*mm + 2*mm   # label + 3 botones
    if cy > content_bot + SOCIAL_MIN_H:
        # Etiqueta
        c.setFillColor(C_MID)
        c.setFont("Helvetica-Oblique", 6)
        c.drawString(lx, cy, "Visitenos en nuestros canales")
        cy -= 3.5 * mm
        ...
        social = [
            # (texto, color_fondo, color_texto, url)
            ("WhatsApp  >",
             colors.HexColor("#25D366"), C_WHITE,
             f"https://wa.me/56933276344?text=Hola%20quiero%20consultar%20el%20modelo%20SKU%3A%20{sku}"),
            ("Instagram  >",
             colors.HexColor("#E1306C"), C_WHITE,
             "https://www.instagram.com/todoexclusivo_shop?igsh=aHg0ZXpxYWNqMGc%3D&utm_source=qr"),
            ("Facebook  >",
             colors.HexColor("#1877F2"), C_WHITE,
             "https://www.facebook.com/share/17FoeykVmr/?mibextid=wwXIfr"),
        ]
        for txt, bg, fg, url_s in social:
            if cy - btn_h < content_bot:
                break
            btn_y = cy - btn_h
            rounded_rect(c, lx, btn_y, btn_w, btn_h, r=2*mm, fill=bg)
            c.setFillColor(fg)
            c.setFont("Helvetica-Bold", 7)
            c.drawCentredString(lx + btn_w / 2, btn_y + 2*mm, txt)
            c.linkURL(url_s, (lx, btn_y, lx + btn_w, btn_y + btn_h), thickness=0)
            cy -= btn_h + btn_g
...
def dest_name(ref_modelo: str) -> str:
    ...
    guion bajo. Esta funcion convierte la referencia del modelo (que puede contener
    guiones para indicar multiples colores, ej: '123-456') en un identificador
    seguro para usar como ancla interna en el documento.

    Parametros:
        ref_modelo (str): Referencia del modelo tal como aparece en el CSV
                          (ej: '123', '123-456').
...
def _layout_prod(carpetas_hd: dict) -> tuple:
    ...
    de fotos son el numero de filas que ocupa la cuadricula de ese color:
    1 fila si tiene N_FOTO (3) fotos o menos, 2 filas si tiene mas de 3.
...
def calcular_bloque_h_con_ph(carpetas_hd: dict, ph: float) -> float:
    ...
    La altura total es la suma de:
    - BLK_PAD: overhead fijo del bloque (header + padding superior e inferior).
    - content_h: altura del area de fotos, calculada como:
        n_grupos x (max_rows x ph + gaps_verticales_entre_filas)
        + gaps_horizontales_entre_grupos_de_color.

    Se usa en agrupar_en_paginas para decidir cuantos modelos caben en una pagina.
...
def agrupar_en_paginas(productos: list) -> list:
    ...
    - Modelos con 3 o mas carpetas de color: siempre ocupan su propia pagina solos,
      porque la cuadricula de fotos requiere todo el espacio vertical disponible.
    - Modelos con 1 o 2 colores: se van acumulando en la pagina actual usando
      el algoritmo greedy (se agrega el siguiente modelo si la suma de alturas
      de todos los modelos de la pagina cabe en la altura util de la pagina).
    - La altura util de pagina se calcula descontando los margenes superior e
      inferior y los gaps entre bloques.

    Parametros:
        productos (list): Lista de dicts de producto, cada uno con las claves
                          'filas' y 'carpetas_hd', en el orden deseado del catalogo.

    Devuelve:
        list[list]: Lista de paginas, donde cada pagina es una lista de productos
                    que caben juntos. Cada elemento de la lista externa es una pagina.
    """
    if not productos:
        return []
    pages   = []
...
def dibujar_indice(cv: canvas.Canvas, productos_info: list):
    ...
    Cada fila de categoria hoja (el nivel mas profundo, ej: 'Ninas' dentro de
    'Ballerinas') es un link clickeable que lleva al primer producto de ese grupo
    en el PDF.

    Estructura visual por nivel jerarquico:
    - Nivel 0 (raiz, ej: 'Ballerinas'): banda de fondo oscuro, texto blanco en mayusculas.
    - Nivel 1 (ej: 'Ninas'): banda de fondo azul oscuro, texto dorado.
    - Nivel 2+: fila de fondo gris claro con prefijo '>>'.
    - Fila hoja: fondo dorado suave (#FDF3DC) con borde izquierdo dorado y flecha '>'.

    Si el indice no cabe en una sola pagina, continua en paginas adicionales
    con indicacion '(continuacion)'.

    Tambien registra bookmarks PDF y entradas en el panel de marcadores del lector
    para cada nivel de la jerarquia.

    Parametros:
        cv (canvas.Canvas): Lienzo PDF sobre el que dibujar.
        productos_info (list): Lista de dicts con claves 'nombre', 'ref', 'categoria'
                               y 'dest' para cada producto del catalogo.
    """
    W, H = A4
    M    = 12 * mm
    ...
    # Mapeo: tuple(cats) -> dest del primer producto de esa categoria
    cat_first_dest: dict = {}
    seen_order: list = []
    for p in entries:
    ...
    # Alturas de linea por nivel de cabecera padre
    LH = {0: 9*mm, 1: 7.5*mm, 2: 7*mm}
    LEAF_H = 8 * mm   # altura de la fila-hoja clickeable
    ...
    def new_index_page(first: bool = False):
        ...
        Parametros:
            first (bool): Si True es la primera pagina del indice y muestra la
                          instruccion de uso; si False muestra '(continuacion)'.
        """
        ...
        label = "Haz clic en una categoria para ir al primer producto" if first else "(continuacion)"
    ...
    for cats in seen_order:
        cats_list  = list(cats)
        leaf_level = len(cats_list) - 1   # indice del ultimo nivel
        leaf_label = cats_list[-1]
        dest       = cat_first_dest[cats]

        # Espacio necesario: cabeceras padre nuevas + fila hoja
        parent_new = sum(
        ...
        # ── Fila hoja: categoria final, clickeable ────────────────────────
        ...
        # Link al primer producto de esta categoria hoja
        cv.linkAbsolute("", dest,
                        Rect=(row_x, row_y, row_x + row_w, row_y + LEAF_H),
                        thickness=0)

        # Outline PDF: la hoja apunta al primer producto
...
URL_TIENDA = "https://todoexclusivo.cl/tienda/"
...
def dibujar_portada(cv: canvas.Canvas):
    ...
    - Boton clickeable "Ver tienda en linea" que enlaza a URL_TIENDA.
    ...
    # Boton "Ver tienda en linea"
    btn_w = 60 * mm
    btn_h = 10 * mm
    btn_x = W / 2 - btn_w / 2
    btn_y = H / 2 - 32*mm
    rounded_rect(cv, btn_x, btn_y, btn_w, btn_h, r=3*mm, fill=C_ACCENT)
    cv.setFillColor(C_DARK)
    cv.setFont("Helvetica-Bold", 10)
    cv.drawCentredString(W / 2, btn_y + 3.5*mm, "Ver tienda en linea  >")
    cv.linkURL(URL_TIENDA,
...
def generar_catalogo(productos: list):
    """
    Orquesta la generacion completa del archivo PDF del catalogo.

    Secuencia de generacion:
    1. Portada (dibujar_portada).
    2. Pagina(s) de indice jerarquico (dibujar_indice).
    3. Paginas de productos: agrupa los productos en paginas (agrupar_en_paginas),
       calcula la altura de cada bloque con photo_h = PHOTO_W fijo, registra
       un bookmark por producto y llama a dibujar_bloque para cada uno.

    El PDF se guarda en OUTPUT_PDF al finalizar con cv.save().

    Parametros:
        productos (list): Lista de dicts de producto con claves:
                          - 'filas' (list): filas del CSV para este modelo.
                          - 'carpetas_hd' (dict): {numero: Path_HD} con las carpetas de fotos.
    """
    ...
    for pagina in paginas:
        ph = PHOTO_W  # tamano fijo, nunca se reduce para meter mas modelos

        # Distribuir los bloques de arriba hacia abajo
...
def cargar_producto(rows: list, ref: str):
    ...
    Si no se encuentra ninguna carpeta HD valida con imagenes, el producto se omite
    (devuelve None) y se imprime una advertencia en consola.
...
def main():
    """
    Funcion principal: carga el inventario y genera el catalogo PDF para vendedores.

    Flujo de ejecucion:
    1. Lee el archivo CSV Inventario_web_precios_al_vendedor.csv usando leer_inventario().
    ...
    4. Para cada ref_modelo, llama a cargar_producto() para buscar carpetas HD
       y construir el dict de producto.
    5. Llama a generar_catalogo() con la lista de productos validos.
    6. El PDF resultante se guarda en OUTPUT_PDF (catalogo.pdf en la carpeta del script).

    No recibe parametros ni devuelve valores. Los errores de lectura del CSV
    lanzan RuntimeError desde leer_inventario(). Los modelos sin carpetas HD
    se omiten silenciosamente con una advertencia en consola.
    """
    ...
    # Obtener refs unicos en orden de aparicion
    refs_vistos: dict = {}
    for row in rows:
